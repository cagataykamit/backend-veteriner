...
|---|---|---|---|---|---|
| Auth | Karışık dönüş modeli (`Result` + doğrudan `Problem` + ad-hoc `ok`) | Hata envelope ve response shape tutarsız | Tüm akışları tek `Result/ProblemDetails` standardına çekme | P0 | Yüksek |
...
### P0 (hemen)
- **Auth:** Controller dönüşlerini tek `Result/ProblemDetails` hattına çekmek.
