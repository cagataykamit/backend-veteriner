...
public sealed class SqlPetOutboxClaimRepository : IPetOutboxClaimRepository
{
    private static readonly string[] PetEventTypes = PetIntegrationEventTypes.All.ToArray();

    private readonly AppDbContext _dbContext;
    private readonly TimeProvider _timeProvider;

    public SqlPetOutboxClaimRepository(AppDbContext dbContext, TimeProvider timeProvider)
    {
    ...
    private static string BuildClaimSql()
    {
        ...
        for (var i = 0; i < PetEventTypes.Length; i++)
        {
            if (i > 0)
                typeInClause.Append(", ");
