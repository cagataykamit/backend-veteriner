...
internal static class PaymentsReportQueryValidation
{
        ...
        CancellationToken ct)
    {
        ...
        {
            return Result<(Guid, Guid?, DateTime, DateTime)>.Failure(
                "Payments.ReportDateRangeInvalid",
        ...
        if (spanDays > PaymentsReportConstants.MaxRangeDays)
        {
            return Result<(Guid, Guid?, DateTime, DateTime)>.Failure(
                "Payments.ReportRangeTooLong",
                $"Tarih aralığı en fazla {PaymentsReportConstants.MaxRangeDays} gün olabilir.");
        }
