...
public sealed class SelectClinicCommandHandler : IRequestHandler<SelectClinicCommand, Result<LoginResultDto>>
{
    ...
    public async Task<Result<LoginResultDto>> Handle(SelectClinicCommand request, CancellationToken ct)
    {
        ...
        if (!tenant.IsActive)
            return Result<LoginResultDto>.Failure("Tenants.TenantInactive", "Pasif kiracı için token üretilemez.");
