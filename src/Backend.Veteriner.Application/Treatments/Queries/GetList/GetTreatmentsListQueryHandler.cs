    ...
    : IRequestHandler<GetTreatmentsListQuery, Result<PagedResult<TreatmentListItemDto>>>
{
        ...
        CancellationToken ct)
    {
        ...
        // tüm kiracı tedavi kayıtlarını DÖNDÜRME. Tenant-wide kullanıcılar dahil, kapsamsız list/okuma engellenir.
        if (requestedClinicId is null)
        {
            return Result<PagedResult<TreatmentListItemDto>>.Failure(
                "Treatments.ClinicScopeRequired",
                "Klinik kapsamı gerekli: aktif klinik bağlamı yok ve clinicId belirtilmedi. Tedaviler klinik kapsamı olmadan listelenemez.");
        }
