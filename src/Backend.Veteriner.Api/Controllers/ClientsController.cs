...
/// <summary>
/// Müşteri (hayvan sahibi) yönetimi. Kiracı JWT <c>tenant_id</c> veya sorgu <c>tenantId</c>.
/// </summary>
[ApiController]
[ApiVersion("1.0")]
...
public sealed class ClientsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetById([FromRoute] Guid id, CancellationToken ct)
    {
        if (!this.TryGetResolvedTenant(_tenantContext, out var tid, out var problem))
            return problem!;
        var result = await _mediator.Send(new GetClientByIdQuery(tid, id), ct);
        return result.ToActionResult(this);
    }
    ...
    public async Task<IActionResult> GetList([FromQuery] PageRequest page, CancellationToken ct)
    {
        if (!this.TryGetResolvedTenant(_tenantContext, out var tid, out var problem))
            return problem!;
        var result = await _mediator.Send(new GetClientsListQuery(tid, page), ct);
        return result.ToActionResult(this);
    }
}
