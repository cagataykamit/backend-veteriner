...
public sealed class UpdatePermissionCommandHandlerTests
{
    ...
    private UpdatePermissionCommandHandler CreateHandler(bool revokeSessions = false)
    {
    ...
    public async Task Handle_Should_ReturnFailure_When_PermissionNotFound()
    {
        ...
        _repo.Verify(r => r.UpdateAsync(It.IsAny<Permission>(), It.IsAny<CancellationToken>()), Times.Never);
    }

    [Fact]
    public async Task Handle_Should_ReturnFailure_When_CodeAlreadyInUse_ByAnotherPermission()
    {
        // Arrange
        var handler = CreateHandler();
        var id = Guid.NewGuid();
        var cmd = new UpdatePermissionCommand(id, "NEWCODE", "desc");

        var existing = new Permission("OLDCODE", "old");
        typeof(Permission).GetProperty("Id")!.SetValue(existing, id);

        _repo.Setup(r => r.GetByIdAsync(id, It.IsAny<CancellationToken>()))
            .ReturnsAsync(existing);

        _repo.Setup(r => r.ExistsByCodeAsync(cmd.Code, It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);
    ...
    [Fact]
    public async Task Handle_Should_UpdatePermission_And_InvalidateCache_When_Successful()
    {
        // Arrange
        var handler = CreateHandler(revokeSessions: false);
        var id = Guid.NewGuid();
        var cmd = new UpdatePermissionCommand(id, "NEWCODE", "new desc");
        ...
        existing.Code.Should().Be("NEWCODE");
