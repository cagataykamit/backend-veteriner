...
public sealed class GetLabResultsListQueryHandlerTests
{
    ...
    public async Task Handle_Should_Fail_When_TenantContextMissing()
    {
        ...
        _labResults.Verify(
            r => r.CountAsync(It.IsAny<ISpecification<LabResult>>(), It.IsAny<CancellationToken>()),
            Times.Never);
    }
    ...
    public async Task Handle_Should_Fail_When_QueryClinic_Differs_From_ActiveClinicContext()
    {
        ...
        var result = await CreateHandler().Handle(new GetLabResultsListQuery(paging, queryClinicId), CancellationToken.None);
    ...
    public async Task Handle_Should_QueryClientsAndPets_When_SearchProvided()
    {
        ...
        _labResults.Setup(r => r.ListAsync(It.IsAny<LabResultsFilteredPagedSpec>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<LabResult>());
        var paging = new PageRequest { Page = 1, PageSize = 20, Search = "  hem  " };
    ...
    public async Task Handle_Should_MapRows_When_ItemsExist()
    {
        ...
        _labResults.Setup(r => r.CountAsync(It.IsAny<LabResultsFilteredCountSpec>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(1);
        _labResults.Setup(r => r.ListAsync(It.IsAny<LabResultsFilteredPagedSpec>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<LabResult> { lr });
        ...
        item.TestName.Should().Be("Biyokimya");
        item.ResultDateUtc.Should().Be(lr.ResultDateUtc);
    ...
    public async Task Handle_Should_ApplyFilterCombination_And_ClampPaging()
    {
        ...
        var result = await CreateHandler().Handle(
            new GetLabResultsListQuery(paging, clinicId, petId, dateFrom, dateTo),
