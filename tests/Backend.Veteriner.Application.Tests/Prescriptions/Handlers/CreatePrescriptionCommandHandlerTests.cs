...
public sealed class CreatePrescriptionCommandHandlerTests
{
    ...
    private static CreatePrescriptionCommand Cmd(
        Guid clinicId,
        Guid petId,
        ...
        => new(
            ...
            examinationId,
            treatmentId,
            prescribedAt ?? DateTime.UtcNow.AddHours(-1),
            "Başlık",
            "İçerik",
            null,
            followUp);
    ...
    public async Task Handle_Should_ReturnFailure_When_DateTooFarInPast()
    {
        ...
        var cmd = Cmd(cid, petId, prescribedAt: DateTime.UtcNow.AddDays(-10));

        var result = await CreateHandler().Handle(cmd, CancellationToken.None);

        result.IsSuccess.Should().BeFalse();
        result.Error.Code.Should().Be("Prescriptions.DateTooFarInPast");
    }
    ...
    public async Task Handle_Should_ReturnFailure_When_DateTooFarInFuture()
    {
        ...
        var cmd = Cmd(cid, petId, prescribedAt: DateTime.UtcNow.AddYears(3));

        var result = await CreateHandler().Handle(cmd, CancellationToken.None);

        result.IsSuccess.Should().BeFalse();
        result.Error.Code.Should().Be("Prescriptions.DateTooFarInFuture");
    }

    [Fact]
    public async Task Handle_Should_ReturnFailure_When_FollowUpBeforePrescription()
    {
        var tid = Guid.NewGuid();
        var cid = Guid.NewGuid();
        var petId = Guid.NewGuid();
        var prescribed = DateTime.UtcNow.AddHours(-1);
        _tenantContext.SetupGet(t => t.TenantId).Returns(tid);
        _clinicContext.SetupGet(c => c.ClinicId).Returns(cid);
        SetupTenantClinicPetSafe(tid, cid, petId);

        var cmd = Cmd(cid, petId, prescribedAt: prescribed, followUp: prescribed.AddHours(-2));

        var result = await CreateHandler().Handle(cmd, CancellationToken.None);

        result.IsSuccess.Should().BeFalse();
        result.Error.Code.Should().Be("Prescriptions.FollowUpBeforePrescription");
    }
    ...
    public async Task Handle_Should_ReturnFailure_When_ExaminationClinicMismatch()
    {
        ...
        result.IsSuccess.Should().BeFalse();
        result.Error.Code.Should().Be("Prescriptions.ExaminationClinicMismatch");
    }
    ...
    public async Task Handle_Should_ReturnFailure_When_ExaminationPetMismatch()
    {
        ...
        result.IsSuccess.Should().BeFalse();
        result.Error.Code.Should().Be("Prescriptions.ExaminationPetMismatch");
    }
    ...
    public async Task Handle_Should_ReturnFailure_When_TreatmentNotFound()
    {
        ...
        var result = await CreateHandler().Handle(Cmd(cid, petId, treatmentId: treatmentId), CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_Should_ReturnFailure_When_TreatmentClinicMismatch()
    {
        var tid = Guid.NewGuid();
        var cid = Guid.NewGuid();
        var petId = Guid.NewGuid();
        ...
        var treatment = new Treatment(tid, Guid.NewGuid(), petId, null, DateTime.UtcNow, "T", "D", null, null);
        ...
        result.IsSuccess.Should().BeFalse();
        result.Error.Code.Should().Be("Prescriptions.TreatmentClinicMismatch");
    }

    [Fact]
    public async Task Handle_Should_ReturnFailure_When_TreatmentPetMismatch()
    {
        var tid = Guid.NewGuid();
        var cid = Guid.NewGuid();
        var petId = Guid.NewGuid();
        ...
        var treatment = new Treatment(tid, cid, Guid.NewGuid(), null, DateTime.UtcNow, "T", "D", null, null);
        typeof(Treatment).GetProperty(nameof(Treatment.Id))!.SetValue(treatment, treatmentId);
        _treatments.Setup(r => r.FirstOrDefaultAsync(It.IsAny<TreatmentByIdSpec>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(treatment);
        ...
        result.IsSuccess.Should().BeFalse();
        result.Error.Code.Should().Be("Prescriptions.TreatmentPetMismatch");
    }

    [Fact]
    public async Task Handle_Should_ReturnFailure_When_ExaminationTreatmentMismatch()
    {
        var tid = Guid.NewGuid();
        var cid = Guid.NewGuid();
        var petId = Guid.NewGuid();
        ...
        var treatment = new Treatment(tid, cid, petId, otherExamId, DateTime.UtcNow, "T", "D", null, null);
        typeof(Treatment).GetProperty(nameof(Treatment.Id))!.SetValue(treatment, treatmentId);
        _treatments.Setup(r => r.FirstOrDefaultAsync(It.IsAny<TreatmentByIdSpec>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(treatment);

        var result = await CreateHandler().Handle(
            Cmd(cid, petId, examinationId: examId, treatmentId: treatmentId),
            CancellationToken.None);

        result.IsSuccess.Should().BeFalse();
        result.Error.Code.Should().Be("Prescriptions.ExaminationTreatmentMismatch");
    }
    ...
    public async Task Handle_Should_Persist_When_Valid()
    {
        ...
        var treatment = new Treatment(tid, cid, petId, examId, DateTime.UtcNow, "T", "D", null, null);
        typeof(Treatment).GetProperty(nameof(Treatment.Id))!.SetValue(treatment, treatmentId);
        _treatments.Setup(r => r.FirstOrDefaultAsync(It.IsAny<TreatmentByIdSpec>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(treatment);
        ...
        var result = await CreateHandler().Handle(
            Cmd(cid, petId, examinationId: examId, treatmentId: treatmentId, followUp: DateTime.UtcNow.AddDays(1)),
            CancellationToken.None);

        result.IsSuccess.Should().BeTrue();
        added.Should().NotBeNull();
        added!.ExaminationId.Should().Be(examId);
        added.TreatmentId.Should().Be(treatmentId);
        _prescriptionsWrite.Verify(r => r.AddAsync(It.IsAny<Prescription>(), It.IsAny<CancellationToken>()), Times.Once);
        _prescriptionsWrite.Verify(r => r.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Once);
    }
}
