...
/// <summary>
/// CQRS-15G: <c>QueryReadModels:PaymentsReportReadEnabled</c> tüm ortamlarda production-safe
/// varsayılan (explicit false) ile gelmelidir.
/// </summary>
public sealed class PaymentsReportReadRoutingOptionsTests
{
    ...
    public void AppSettings_Should_KeepPaymentsReportReadDisabled(string fileName)
    {
        using var document = LoadAppSettings(fileName);
        var enabled = document.RootElement
            .GetProperty("QueryReadModels")
            .GetProperty("PaymentsReportReadEnabled")
            .GetBoolean();

        enabled.Should().BeFalse($"{fileName} QueryReadModels:PaymentsReportReadEnabled default false olmalı");
    }
