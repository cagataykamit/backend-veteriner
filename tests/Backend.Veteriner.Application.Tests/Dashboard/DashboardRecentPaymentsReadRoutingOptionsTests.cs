...
/// <summary>
/// CQRS-15B: <c>QueryReadModels:DashboardRecentPaymentsReadEnabled</c> tüm ortamlarda production-safe
/// varsayılan (explicit false) ile gelmelidir.
/// </summary>
public sealed class DashboardRecentPaymentsReadRoutingOptionsTests
{
    ...
    public void AppSettings_Should_KeepDashboardRecentPaymentsReadDisabled(string fileName)
    {
        using var document = LoadAppSettings(fileName);
        var enabled = document.RootElement
            .GetProperty("QueryReadModels")
            .GetProperty("DashboardRecentPaymentsReadEnabled")
            .GetBoolean();

        enabled.Should().BeFalse($"{fileName} QueryReadModels:DashboardRecentPaymentsReadEnabled default false olmalı");
    }
