...
/// <summary>
/// CQRS-15E: <c>QueryReadModels:ClientPaymentSummaryReadEnabled</c> tüm ortamlarda production-safe
/// varsayılan (explicit false) ile gelmelidir.
/// </summary>
public sealed class ClientPaymentSummaryReadRoutingOptionsTests
{
    ...
    public void AppSettings_Should_KeepClientPaymentSummaryReadDisabled(string fileName)
    {
        using var document = LoadAppSettings(fileName);
        var enabled = document.RootElement
            .GetProperty("QueryReadModels")
            .GetProperty("ClientPaymentSummaryReadEnabled")
            .GetBoolean();

        enabled.Should().BeFalse($"{fileName} QueryReadModels:ClientPaymentSummaryReadEnabled default false olmalı");
    }
