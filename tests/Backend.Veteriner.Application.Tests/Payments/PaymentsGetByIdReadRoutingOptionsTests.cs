...
/// <summary>
/// CQRS-16B: <c>QueryReadModels:PaymentsGetByIdReadEnabled</c> tüm ortamlarda production-safe varsayılan
/// (explicit false) ile gelmelidir.
/// </summary>
public sealed class PaymentsGetByIdReadRoutingOptionsTests
{
    ...
    public void AppSettings_Should_KeepPaymentsGetByIdReadDisabled(string fileName)
    {
        using var document = LoadAppSettings(fileName);
        var enabled = document.RootElement
            .GetProperty("QueryReadModels")
            .GetProperty("PaymentsGetByIdReadEnabled")
            .GetBoolean();

        enabled.Should().BeFalse($"{fileName} QueryReadModels:PaymentsGetByIdReadEnabled default false olmalı");
    }
