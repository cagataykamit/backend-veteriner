...
/// <summary>
/// CQRS-14E: <c>QueryReadModels:PaymentsListReadEnabled</c> tüm ortamlarda production-safe varsayılan
/// (explicit false) ile gelmelidir. Rollout yalnızca bilinçli flag açma ile yapılır.
/// </summary>
public sealed class PaymentsListReadRoutingOptionsTests
{
    ...
    public void AppSettings_Should_KeepPaymentsListReadDisabled(string fileName)
    {
        using var document = LoadAppSettings(fileName);
        var enabled = document.RootElement
            .GetProperty("QueryReadModels")
            .GetProperty("PaymentsListReadEnabled")
            .GetBoolean();

        enabled.Should().BeFalse($"{fileName} QueryReadModels:PaymentsListReadEnabled default false olmalı");
    }
