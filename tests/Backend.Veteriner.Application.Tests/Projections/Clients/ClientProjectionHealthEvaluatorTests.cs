...
public sealed class ClientProjectionHealthEvaluatorTests
{
    private static readonly ClientProjectionHealthOptions DefaultHealth = new()
    {
    ...
    private static readonly QueryReadModelsOptions FlagOff = new() { ClientsEnabled = false };
    ...
    public void Evaluate_Should_BeHealthy_WhenNoPendingAndQueryReachable()
    {
        ...
        var result = ClientProjectionHealthEvaluator.Evaluate(status, DefaultHealth, FlagOff);

        result.Level.Should().Be(ClientProjectionHealthLevel.Healthy);
    }
    ...
    public void Evaluate_Should_BeUnhealthy_WhenQueryDbUnreachable()
    {
        ...
        result.Level.Should().Be(ClientProjectionHealthLevel.Unhealthy);
    }
    ...
    public void Evaluate_Should_BeDegraded_WhenPendingAgeAtDegradedThreshold()
    {
        ...
        result.Level.Should().Be(ClientProjectionHealthLevel.Degraded);
    }
    ...
    public void Evaluate_Should_BeDegraded_WhenReadFlagOnAndProjectionDisabledWithoutPending()
    {
        ...
        var flags = new QueryReadModelsOptions { ClientsEnabled = true };

        var result = ClientProjectionHealthEvaluator.Evaluate(status, DefaultHealth, flags);

        result.Level.Should().Be(ClientProjectionHealthLevel.Degraded);
        result.Data["clientsReadEnabled"].Should().Be(true);
    ...
    public void Evaluate_Should_IncludeOperationalDataPayload()
    {
        ...
        result.Data.Should().ContainKey("clientsReadEnabled");
    }

    private static ClientProjectionStatus CreateStatus(
        int pendingCount = 0,
