...
public sealed class GetBreedListQueryHandlerTests
{
    ...
    private GetBreedListQueryHandler CreateHandler()
    {
        _cache.Setup(c => c.TryGetBreedsList(
                It.IsAny<bool?>(),
                It.IsAny<Guid?>(),
                It.IsAny<string?>(),
                It.IsAny<int>(),
                It.IsAny<int>(),
                out It.Ref<PagedResult<BreedListItemDto>?>.IsAny))
            .Returns(false);
    ...
    {
        var cached = PagedResult<BreedListItemDto>.Create(
            [new BreedListItemDto(Guid.NewGuid(), Guid.NewGuid(), "Köpek", "Golden", true)],
            1,
            1,
            20);

        _cache.Setup(c => c.TryGetBreedsList(null, null, null, 1, 20, out cached!))
            .Returns(true);

        var handler = new GetBreedListQueryHandler(_read.Object, _cache.Object);
        var result = await handler.Handle(
            new GetBreedListQuery(new PageRequest { Page = 1, PageSize = 20 }, null, null, null),
            CancellationToken.None);
        ...
        _read.Verify(r => r.CountAsync(It.IsAny<BreedsCountSpec>(), It.IsAny<CancellationToken>()), Times.Never);
    }
    ...
    public async Task Handle_Should_StoreInCache_When_CacheMiss()
    {
        ...
        _cache.Verify(c => c.SetBreedsList(null, null, null, 1, 20, It.IsAny<PagedResult<BreedListItemDto>>()), Times.Once);
    }
}
