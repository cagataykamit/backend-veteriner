...
internal static class IntegrationTestAuthHelper
{
    ...
    /// <summary>
    /// Tenant-wide olmayan kullanıcı: examination create/update izinleri;
    /// yalnız atandığı klinikte yazma yapabilmeli.
    /// </summary>
    public static async Task<(string Email, string Password, Guid AssignedClinicId, Guid UnassignedClinicId)>
        SeedExaminationWriterUserAsync(IServiceProvider services, IPasswordHasher hasher)
    {
        await EnsureRolePermissionBindingsAsync(services);
        ...
        var unassignedClinic = new Clinic(tenant.Id, $"ExamWriter-{Guid.NewGuid():N}"[..14], "Konya");
        db.Clinics.Add(unassignedClinic);
        await db.SaveChangesAsync();

        var createClaim = await EnsureIntegrationPermissionClaimAsync(
            db, "IntegrationExaminationCreator", PermissionCatalog.Examinations.Create, "Examinations");
        var updateClaim = await EnsureIntegrationPermissionClaimAsync(
            db, "IntegrationExaminationUpdater", PermissionCatalog.Examinations.Update, "Examinations");

        var email = $"exam-writer-{Guid.NewGuid():N}@example.com";
        const string password = "123456";
        var user = new User(email, hasher.Hash(password));
        db.Users.Add(user);
        await db.SaveChangesAsync();
    ...
    /// <summary>
    /// Tenant-wide olmayan kullanıcı: <c>Treatments.Create</c> ve <c>Treatments.Update</c> iznine sahip,
    /// Admin / Owner / PlatformAdmin / ClinicAdmin claim'i yok. Yalnız atandığı kliniğe tedavi yazabilmeli.
    /// </summary>
    public static async Task<(string Email, string Password, Guid AssignedClinicId, Guid UnassignedClinicId)>
        SeedTreatmentWriterUserAsync(IServiceProvider services, IPasswordHasher hasher)
    {
        await EnsureRolePermissionBindingsAsync(services);
        ...
        var unassignedClinic = new Clinic(tenant.Id, $"TreatWriter-{Guid.NewGuid():N}"[..14], "Konya");
        db.Clinics.Add(unassignedClinic);
        await db.SaveChangesAsync();

        var createClaim = await EnsureIntegrationPermissionClaimAsync(
            db, "IntegrationTreatmentCreator", PermissionCatalog.Treatments.Create, "Treatments");
        var updateClaim = await EnsureIntegrationPermissionClaimAsync(
            db, "IntegrationTreatmentUpdater", PermissionCatalog.Treatments.Update, "Treatments");

        var email = $"treat-writer-{Guid.NewGuid():N}@example.com";
        const string password = "123456";
        var user = new User(email, hasher.Hash(password));
        db.Users.Add(user);
        await db.SaveChangesAsync();
    ...
    /// <summary>
    /// Tenant-wide olmayan kullanıcı: <c>Prescriptions.Create</c> ve <c>Prescriptions.Update</c> iznine sahip,
    /// Admin / Owner / PlatformAdmin / ClinicAdmin claim'i yok. Yalnız atandığı kliniğe reçete yazabilmeli.
    /// </summary>
    public static async Task<(string Email, string Password, Guid AssignedClinicId, Guid UnassignedClinicId)>
        SeedPrescriptionWriterUserAsync(IServiceProvider services, IPasswordHasher hasher)
    {
        await EnsureRolePermissionBindingsAsync(services);
        ...
        var unassignedClinic = new Clinic(tenant.Id, $"PrescWriter-{Guid.NewGuid():N}"[..14], "Konya");
        db.Clinics.Add(unassignedClinic);
        await db.SaveChangesAsync();

        var createClaim = await EnsureIntegrationPermissionClaimAsync(
            db, "IntegrationPrescriptionCreator", PermissionCatalog.Prescriptions.Create, "Prescriptions");
        var updateClaim = await EnsureIntegrationPermissionClaimAsync(
            db, "IntegrationPrescriptionUpdater", PermissionCatalog.Prescriptions.Update, "Prescriptions");

        var email = $"presc-writer-{Guid.NewGuid():N}@example.com";
        const string password = "123456";
        var user = new User(email, hasher.Hash(password));
        db.Users.Add(user);
        await db.SaveChangesAsync();
    ...
    /// <summary>
    /// Tenant-wide olmayan kullanıcı: lab result create/update izinlerine sahip, Admin / Owner / PlatformAdmin /
    /// ClinicAdmin claim'i yok. Yalnız atandığı kliniğe lab result yazabilmeli.
    /// </summary>
    public static async Task<(string Email, string Password, Guid AssignedClinicId, Guid UnassignedClinicId)>
        SeedLabResultWriterUserAsync(IServiceProvider services, IPasswordHasher hasher)
    {
        await EnsureRolePermissionBindingsAsync(services);
        ...
        var unassignedClinic = new Clinic(tenant.Id, $"LabWriter-{Guid.NewGuid():N}"[..14], "Konya");
        db.Clinics.Add(unassignedClinic);
        await db.SaveChangesAsync();

        var createClaim = await EnsureIntegrationPermissionClaimAsync(
            db, "IntegrationLabResultCreator", PermissionCatalog.LabResults.Create, "LabResults");
        var updateClaim = await EnsureIntegrationPermissionClaimAsync(
            db, "IntegrationLabResultUpdater", PermissionCatalog.LabResults.Update, "LabResults");

        var email = $"lab-writer-{Guid.NewGuid():N}@example.com";
        const string password = "123456";
        var user = new User(email, hasher.Hash(password));
        db.Users.Add(user);
        await db.SaveChangesAsync();
